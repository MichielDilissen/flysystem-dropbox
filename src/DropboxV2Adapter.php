...
class DropboxV2Adapter extends AbstractAdapter
{
    ...
    public function readStream($path)
    {
        ...
        if ( ! $this->client->download($location, $stream)) {
            fclose($stream);
    ...
    public function listContents($directory = '', $recursive = false)
    {
        ...
        if ( ! $result = $this->client->listFolder($location)) {
            return [];
        }
        //var_dump($result); die();
    ...
    {
        $location = $this->applyPathPrefix($path);
        if ( ! $result = $this->client->upload($path, $contents)) {
    ...
    protected function uploadStream($path, $resource, WriteMode $mode)
    {
        ...
        if ( ! $result = $this->client->upload($location, $resource)) {
